...
$(document).ready(() => {
    ...
    jobpostapproval_table = $('.datatables-applicants').DataTable({

        ajax: {
            url: `${baseUrl}/Candidate/GetJobApplicants?jpId=${jpid}`,
            dataSrc: function (resp) {
                if (resp.success === true) {
                    //for testing only score are randomly generated, remove this in with real scores from server
                    resp.data.forEach(item => {
                        item.fitScore = Math.floor(Math.random() * 100) + 30;
                    });
        ...
        columns: [
            ...
            { data: 'appliedDate', title: 'Applied On' },
            {
                data: 'fitScore',
                title: 'Fit Score',
                orderable: false,
                render: function (data, type, row) {
                    ...
                    let color = '';
                    if (score >= 85) color = '#28a745';          // success green
                    else if (score >= 75) color = '#88d498';     // pale green
                    else if (score >= 60) color = '#ffc107';     // yellowish
                    else if (score >= 40) color = '#ff6b6b';     // light red
                    else color = '#c0392b';                      // dark red

                    return `
                    <div class="circle-wrapper" style="--score: ${score}; --bar-color: ${color};">
                      <svg class="circular-chart" viewBox="0 0 36 36">
                        ...
                        <text x="18" y="20.5" class="percentage">${score}%</text>
                      </svg>
                    </div>
                    `;
                }
